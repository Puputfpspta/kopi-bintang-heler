...
function addOrder($user_id, $name, $phone, $address, $house_number, $postal_code, $city, $courier) {
    global $conn;
    try {
        // Ambil email dari tabel users berdasarkan user_id
        ...
        // Masukkan pesanan baru ke tabel orders
        ...
        $stmt->bind_param("isssssss", $user_id, $name, $email, $phone, $address, $house_number, $postal_code, $city, $courier);
        if (!$stmt->execute()) {
            throw new Exception("Execute failed: " . $stmt->error);
        }
        $stmt->close();
